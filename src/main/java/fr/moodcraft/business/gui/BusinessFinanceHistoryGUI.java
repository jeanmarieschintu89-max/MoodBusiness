...
public final class BusinessFinanceHistoryGUI {
    ...
    public static final String TITLE = GuiTitle.of("Logs financiers");
    ...
    public static void open(Player p, Business business) {
        ...
        List<FinanceTransaction> transactions = FinanceStorage.getByBusiness(business.getId());
        ...
        if (transactions.isEmpty()) {
            SafeGUI.set(inv, 22, new ItemBuilder(Material.PAPER)
    ...
    private static String targetLine(FinanceTransaction transaction) {
        if (transaction.getTargetName() == null || transaction.getTargetName().isBlank()) {
