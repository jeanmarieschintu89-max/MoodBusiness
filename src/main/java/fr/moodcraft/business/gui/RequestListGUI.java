...
public final class RequestListGUI {
    ...
    public static final String TITLE_PUBLIC = GuiTitle.of("Missions disponibles");
    public static final String TITLE_MY = GuiTitle.of("Mes missions");
    ...
    private static void open(Player p, String title, List<BusinessRequest> list, boolean ownList) {
        ...
        SafeGUI.set(inv, 4,
                new ItemBuilder(ownList ? Material.WRITABLE_BOOK : Material.BOOK)
                        .name(ownList ? "§6✦ §fMes missions §6✦" : "§6✦ §fMissions disponibles §6✦")
                        .lore(
                                "§8• §7Total : §e" + list.size(),
                                "§8• §7Service : §aMood§6Craft",
                                "",
                                ownList
                                        ? "§e➜ §fOuvrez une mission pour l'annuler"
                                        : "§e➜ §fSélectionnez une mission"
                        )
                        .build()
        );
            ...
            SafeGUI.set(inv, SLOTS[index],
                    new ItemBuilder(request.getStatus().isOpen() ? Material.PAPER : Material.GRAY_DYE)
                            ...
                            .lore(
                                    ...
                                    "§8• §7Expire : §f" + shortDate(request.getExpiresAt()),
    ...
    private static String shortText(String text, int max) {
        if (text == null || text.isBlank()) return "Sans nom";
